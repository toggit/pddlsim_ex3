...
(:action kick-left
	...
	:precondition
  	(and (ball ?b)(at-ball ?b ?c1) (at-robby ?c1) (connected ?c1 ?c2) (left-foot ?f) (not(not-up ?f)))
 :effect
	  (and (at-ball ?b ?c2) (not (at-ball ?b ?c1)) (not-up ?f))
		)

(:action kick-right
	:parameters (?b ?c1 ?c2 ?f)
	:precondition
  	(and (ball ?b) (at-ball ?b ?c1) (at-robby ?c1) (connected ?c1 ?c2) (right-foot ?f) (not(not-up ?f)))
 :effect
	  (and (at-ball ?b ?c2) (not (at-ball ?b ?c1)) (not-up ?f))
	)
)